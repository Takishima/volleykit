...
const PDF_LANGUAGES: Array<{ code: Language; name: string }> = [
...
export function PdfLanguageModal({
  ...
  // Reset to default when modal opens - valid use case for syncing with prop
  useEffect(() => {
    if (isOpen) {
      setSelectedLanguage(defaultLanguage); // eslint-disable-line react-hooks/set-state-in-effect -- reset state when modal opens is valid
    }
  ...
  return (
    ...
    >
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="pdf-language-modal-title"
      >
