...
describe('TravelTimeBadge', () => {
  describe('loading state', () => {
    it('shows loading indicator when isLoading is true', () => {
      const { container } = render(<TravelTimeBadge durationMinutes={undefined} isLoading={true} />)
  ...
  describe('error state', () => {
    it('returns null when isError is true', () => {
      const { container } = render(<TravelTimeBadge durationMinutes={30} isError={true} />)

      expect(container.firstChild).toBeNull()
    })

    it('returns null when isError is true even with duration', () => {
      const { container } = render(<TravelTimeBadge durationMinutes={45} isError={true} />)
  ...
  describe('undefined duration', () => {
    it('returns null when durationMinutes is undefined', () => {
      const { container } = render(<TravelTimeBadge durationMinutes={undefined} />)
    ...
    it('returns null when durationMinutes is undefined and not loading', () => {
      const { container } = render(
        <TravelTimeBadge durationMinutes={undefined} isLoading={false} isError={false} />
      )
  ...
  describe('styling', () => {
    it('applies neutral variant', () => {
      ...
      const badge = container.querySelector('[class*="neutral"]') || container.firstChild
      expect(badge).toBeInTheDocument()
    })
    ...
    it('combines custom className with component classes', () => {
      const { container } = render(<TravelTimeBadge durationMinutes={30} className="mt-2" />)
