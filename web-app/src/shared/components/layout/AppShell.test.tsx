...
import { useAuthStore, type UserProfile } from '@/shared/stores/auth'
...
function createMockAuthStore(overrides: Record<string, unknown> = {}) {
...
describe('AppShell', () => {
  ...
  describe('demo mode banner', () => {
    it('renders demo banner when dataSource is demo', () => {
      ...
      expect(screen.getByText('Demo Mode - Viewing sample data')).toBeInTheDocument()
    })
    ...
    it('does NOT render demo banner when dataSource is api', () => {
      ...
      expect(screen.queryByText('Demo Mode - Viewing sample data')).not.toBeInTheDocument()
    })
    ...
    it('has proper accessibility attributes', () => {
      ...
      const banner = screen.getByText('Demo Mode - Viewing sample data').closest('[role="alert"]')
    ...
    ])('displays correct translation for $locale locale', ({ locale, expected }) => {
      setLocale(locale)
  ...
  describe('occupation dropdown', () => {
    it('does NOT render dropdown when user has only one occupation', () => {
      ...
      expect(screen.queryByRole('button', { name: /referee/i })).not.toBeInTheDocument()
    })
    ...
    it('does NOT render dropdown when user has no occupations', () => {
      ...
      expect(screen.queryByRole('button', { name: /select role/i })).not.toBeInTheDocument()
    })
  })
  ...
  describe('calendar mode', () => {
    ...
    describe('navigation filtering', () => {
      it('hides Compensations nav item in calendar mode', () => {
        ...
        expect(screen.queryByRole('link', { name: /compensation/i })).not.toBeInTheDocument()
      })
      ...
      it('hides Exchange nav item in calendar mode', () => {
        ...
        expect(screen.queryByRole('link', { name: /exchange/i })).not.toBeInTheDocument()
      })
      ...
      it('shows Assignments nav item in calendar mode', () => {
        ...
        expect(screen.getByRole('link', { name: /assignment/i })).toBeInTheDocument()
      })
      ...
      it('shows all nav items when NOT in calendar mode', () => {
        ...
        expect(screen.getByRole('link', { name: /assignment/i })).toBeInTheDocument()
        expect(screen.getByRole('link', { name: /compensation/i })).toBeInTheDocument()
