...
vi.mock('@/shared/utils/date-helpers', () => ({
...
describe('prefetchAllTabData', () => {
  ...
  it('continues prefetching other queries when one fails', async () => {
    ...
    // First query fails
    vi.mocked(api.searchAssignments).mockRejectedValue(new Error('Network error'))
    // Rest succeed
    vi.mocked(api.searchCompensations).mockResolvedValue({
      items: [],
      totalItemsCount: 0,
  ...
  it('handles all queries failing gracefully', async () => {
    ...
    vi.mocked(api.getAssociationSettings).mockRejectedValue(new Error('Error 4'))
