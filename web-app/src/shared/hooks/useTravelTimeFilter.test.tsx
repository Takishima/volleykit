...
import type { TravelTimeResult, Coordinates } from '@/shared/services/transport'
...
vi.mock('@/api/queryKeys', () => ({
  queryKeys: {
    travelTime: {
      hall: vi.fn((hallId, homeHash, dayType) => ['travelTime', hallId, homeHash, dayType]),
    },
  },
}))
...
describe('useTravelTimeFilter', () => {
  ...
  describe('when transport is enabled', () => {
    ...
    it('calculates travel time for exchanges', async () => {
      const { calculateTravelTime } = await import('@/shared/services/transport')
  ...
  describe('filtering', () => {
    ...
    it('returns all exchanges when filtering is disabled', async () => {
      ...
      const exchanges = [createMockExchange('hall-1', { lat: 46.0, lon: 7.0 })]
  ...
  describe('filterByTravelTime function', () => {
    it('returns true for exchanges under limit', async () => {
      ...
      const { result } = renderHook(() => useTravelTimeFilter([mockExchange]), {
        wrapper: createWrapper(),
      })
