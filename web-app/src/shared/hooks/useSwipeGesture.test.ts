...
describe('useSwipeGesture', () => {
  ...
  describe('enabled option', () => {
    it('should not start drag when disabled', () => {
      ...
      const { result } = renderHook(() => useSwipeGesture({ enabled: false, onDragStart }))
    ...
    it('should start drag when enabled', () => {
      const onDragStart = vi.fn()
      const { result } = renderHook(() => useSwipeGesture({ enabled: true, onDragStart }))
  ...
  describe('onDragEnd callback', () => {
    it('should reset position when onDragEnd returns false', async () => {
      ...
      const { result } = renderHook(() => useSwipeGesture({ onDragEnd, threshold: 50 }))
