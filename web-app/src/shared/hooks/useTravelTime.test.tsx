...
describe('useTravelTime', () => {
  ...
  describe('production mode with OJP API', () => {
    it('uses real transport API when OJP is configured', async () => {
      const { calculateTravelTime, isOjpConfigured } = await import('@/shared/services/transport')
      const { useAuthStore } = await import('@/shared/stores/auth')
      const { useSettingsStore } = await import('@/shared/stores/settings')
...
describe('day type caching', () => {
  ...
  it('uses getDayType to determine cache key', async () => {
    const { getDayType, calculateMockTravelTime } = await import('@/shared/services/transport')
    const { useAuthStore } = await import('@/shared/stores/auth')
    const { useSettingsStore } = await import('@/shared/stores/settings')
