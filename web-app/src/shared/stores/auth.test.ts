...
describe('useAuthStore', () => {
  ...
  describe('login', () => {
    ...
    it('succeeds via fallback when redirected flag is missing but CSRF token present', async () => {
      ...
      expect(result).toBe(true)
      expect(useAuthStore.getState().status).toBe('authenticated')
      expect(setCsrfToken).toHaveBeenCalledWith('fallback-csrf-token-12345678901234')
    })
    ...
    it('handles already authenticated user (login page has CSRF token)', async () => {
      ...
      mockFetch.mockResolvedValueOnce({
        ...
        redirected: true,
        text: () =>
          Promise.resolve(createDashboardHtmlWithReferee('existing-session-csrf-token-1234')),
      })
    ...
    it('sends correct form data to authentication endpoint', async () => {
      ...
      expect(url).toContain('/sportmanager.security/authentication/authenticate')
  ...
  describe('checkSession', () => {
    ...
    it('preserves groupedEligibleAttributeValues when dashboard has no activeParty data', async () => {
      ...
      const existingEligibleValues = [{ __identity: 'attr-3', roleIdentifier: 'Referee' }]
    ...
    it('returns false and sets idle status on timeout', async () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith('[VolleyKit][App]', 'Session check timed out')
