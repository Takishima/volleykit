...
describe('useTourStore', () => {
  ...
  describe('startTour', () => {
    ...
    it('can start different tours', () => {
      const tourIds: TourId[] = ['assignments', 'compensations', 'exchange', 'settings']
  ...
  describe('completeTour', () => {
    ...
    it('does not duplicate tour in completedTours', () => {
      ...
      expect(useTourStore.getState().completedTours.filter((t) => t === 'assignments').length).toBe(
        1
      )
    })
    ...
    it('removes tour from dismissedTours if it was there', () => {
      ...
      expect(useTourStore.getState().dismissedTours).not.toContain('assignments')
      expect(useTourStore.getState().completedTours).toContain('assignments')
    })
  ...
  describe('dismissTour', () => {
    ...
    it('does not duplicate tour in dismissedTours', () => {
      ...
      expect(useTourStore.getState().dismissedTours.filter((t) => t === 'exchange').length).toBe(1)
    })
  ...
  describe('shouldShowTour', () => {
    ...
    it('returns false for dismissed tours', () => {
      ...
      expect(useTourStore.getState().shouldShowTour('compensations')).toBe(false)
    })
  ...
  describe('getTourStatus', () => {
    it('returns "completed" for completed tours', () => {
      ...
      expect(useTourStore.getState().getTourStatus('assignments')).toBe('completed')
    })

    it('returns "dismissed" for dismissed tours', () => {
      useTourStore.setState({ dismissedTours: ['compensations'] })

      expect(useTourStore.getState().getTourStatus('compensations')).toBe('dismissed')
    })

    it('returns "not_started" for tours that have not been started', () => {
      expect(useTourStore.getState().getTourStatus('exchange')).toBe('not_started')
    })
    ...
    it('prioritizes "completed" over "dismissed"', () => {
      ...
      expect(useTourStore.getState().getTourStatus('settings')).toBe('completed')
    })
  })
