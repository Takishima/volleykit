 ...
 */
function compareValues(valueA: unknown, valueB: unknown, descending: boolean): number {
...
export const mockApi = {
  ...
  async getAssignmentDetails(convocationId: string, _properties: string[]): Promise<Assignment> {
    ...
    const store = useDemoStore.getState()
    const assignment = store.assignments.find((a) => a.__identity === convocationId)
  ...
  async getCompensationDetails(compensationId: string): Promise<ConvocationCompensationDetailed> {
    ...
    const demoCompensation = compensation.convocationCompensation as DemoConvocationCompensation
  ...
  async getGameWithScoresheet(gameId: string): Promise<GameDetails> {
    ...
    const assignment = store.assignments.find((a) => a.refereeGame?.game?.__identity === gameId)
  ...
  async updateNominationList(
    ...
    const gameNominations = store.nominationLists[gameId]
    if (gameNominations) {
      const team = gameNominations.home?.__identity === nominationListId ? 'home' : 'away'
