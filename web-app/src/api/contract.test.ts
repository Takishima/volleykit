...
describe('Mock data contract tests', () => {
  ...
  describe('Compensations', () => {
    it('each compensation passes schema validation', () => {
      ...
      compensations.forEach((compensation, index) => {
        ...
        if (!result.success) {
          throw validationError('Compensation', index, result.error, compensation)
        }
        expect(result.success).toBe(true)
      })
    })
  ...
  describe('Association switching', () => {
    it('SV association data passes validation', () => {
      ...
      const { assignments, compensations, exchanges } = useDemoStore.getState()
      ...
      expect(compensationRecordSchema.safeParse(compensations[0]).success).toBe(true)
...
describe('Assignment details endpoint', () => {
  ...
  it('getAssignmentDetails returns valid assignment', async () => {
    ...
    const details = await mockApi.getAssignmentDetails(firstAssignment!.__identity, [
  ...
  it('getAssignmentDetails throws for non-existent assignment', async () => {
    await expect(mockApi.getAssignmentDetails('non-existent-id', [])).rejects.toThrow(
...
describe('Compensation details endpoint', () => {
  ...
  it('getCompensationDetails throws for non-existent compensation', async () => {
    await expect(mockApi.getCompensationDetails('non-existent-id')).rejects.toThrow(
...
describe('Exchange mutation endpoints', () => {
  ...
  it('applyForExchange removes exchange and creates assignment', async () => {
    ...
    const { exchanges: updated, assignments: updatedAssignments } = useDemoStore.getState()

    // Exchange should be removed from the list
    const exchangeStillExists = updated.find((e) => e.__identity === openExchange!.__identity)
...
describe('Settings endpoints', () => {
  ...
  it('getAssociationSettings returns valid settings object', async () => {
    ...
    expect(settings).toHaveProperty('hoursAfterGameStartForRefereeToEditGameList')
    expect(typeof settings.hoursAfterGameStartForRefereeToEditGameList).toBe('number')
  })
...
describe('Nomination list endpoints', () => {
  ...
  it('getPossiblePlayerNominations returns valid nominations', async () => {
    ...
    const response = await mockApi.getPossiblePlayerNominations(nominationList.__identity!)
...
describe('Scoresheet and game details endpoints', () => {
  ...
  it('updateScoresheet returns updated scoresheet', async () => {
    ...
    const result = await mockApi.updateScoresheet(`scoresheet-${gameId}`, gameId, scorerId, false)
  ...
  it('finalizeScoresheet marks game as validated in store', async () => {
    ...
    await mockApi.finalizeScoresheet(`scoresheet-${gameId}`, gameId, scorerId, 'resource-123')
...
describe('File upload endpoint', () => {
  it('uploadResource rejects invalid file types', async () => {
    ...
    await expect(mockApi.uploadResource(invalidFile)).rejects.toThrow('Invalid file type')
  })
  ...
  it('uploadResource rejects files that are too large', async () => {
    ...
    await expect(mockApi.uploadResource(largeFile)).rejects.toThrow('File too large')
  })
...
describe('Filtering and pagination', () => {
  ...
  it('searchAssignments respects offset parameter', async () => {
    ...
    expect(offsetResponse.items[0]?.__identity).toBe(fullResponse.items[1]?.__identity)
  })
  ...
  it('searchCompensations sorts by date descending', async () => {
    const response = await mockApi.searchCompensations({
      propertyOrderings: [{ propertyName: 'compensationDate', descending: true }],
    })
