...
describe('extractCalendarCode', () => {
  ...
  describe('URL extraction - https://volleymanager.volleyball.ch/calendar/', () => {
    it('extracts code from https URL', () => {
      expect(extractCalendarCode('https://volleymanager.volleyball.ch/calendar/ABC123')).toBe(
        'ABC123'
      )
    })
    ...
    it('extracts code from URL with www prefix', () => {
      expect(extractCalendarCode('https://www.volleymanager.volleyball.ch/calendar/ABC123')).toBe(
        'ABC123'
      )
    })
  ...
  describe('URL extraction - webcal://', () => {
    it('extracts code from webcal URL', () => {
      expect(extractCalendarCode('webcal://volleymanager.volleyball.ch/calendar/ABC123')).toBe(
        'ABC123'
      )
    })

    it('extracts code from webcal URL with www prefix', () => {
      expect(extractCalendarCode('webcal://www.volleymanager.volleyball.ch/calendar/ABC123')).toBe(
        'ABC123'
      )
    })
  })
  ...
  describe('URL extraction - indoor/iCal/referee/', () => {
    it('extracts code from indoor iCal referee path', () => {
      expect(
        extractCalendarCode('https://volleymanager.volleyball.ch/indoor/iCal/referee/ABC123')
      ).toBe('ABC123')
    })
    ...
    it('extracts code from webcal indoor iCal referee path', () => {
      expect(
        extractCalendarCode('webcal://volleymanager.volleyball.ch/indoor/iCal/referee/ABC123')
      ).toBe('ABC123')
    })

    it('extracts code regardless of case in path', () => {
      expect(
        extractCalendarCode('https://volleymanager.volleyball.ch/indoor/iCal/referee/BLYMVF')
      ).toBe('BLYMVF')
    })
  })
  ...
  describe('invalid URLs', () => {
    it('returns null for URLs from other domains', () => {
      expect(extractCalendarCode('https://example.com/calendar/ABC123')).toBeNull()
      expect(extractCalendarCode('https://volleyball.ch/calendar/ABC123')).toBeNull()
    })

    it('returns null for URLs with invalid paths', () => {
      expect(extractCalendarCode('https://volleymanager.volleyball.ch/other/ABC123')).toBeNull()
    })

    it('returns null for URLs with invalid code in path', () => {
      expect(extractCalendarCode('https://volleymanager.volleyball.ch/calendar/TOOLONG')).toBeNull()
      expect(extractCalendarCode('https://volleymanager.volleyball.ch/calendar/SHORT')).toBeNull()
    })
  })
  ...
  describe('iOS/Safari edge cases', () => {
    ...
    it('handles URLs with fragments via fallback', () => {
      expect(
        extractCalendarCode('https://volleymanager.volleyball.ch/indoor/iCal/referee/ABC123#top')
      ).toBe('ABC123')
    })
...
describe('validateCalendarCode', () => {
  ...
  describe('API validation', () => {
    it('returns valid when API responds with 200', async () => {
      ...
      expect(mockFetch).toHaveBeenCalledWith('https://api.example.com/iCal/referee/ABC123', {
        method: 'HEAD',
        signal: undefined,
      })
    })
...
describe('mapCalendarAssignmentToAssignment', () => {
  ...
  describe('basic mapping', () => {
    ...
    it('maps startTime to game startingDateTime', () => {
      ...
      expect(result.refereeGame?.game?.startingDateTime).toBe('2025-02-15T19:30:00+01:00')
    })
  ...
  describe('team mapping', () => {
    it('maps homeTeam to encounter teamHome name', () => {
      ...
      expect(result.refereeGame?.game?.encounter?.teamHome?.name).toBe('VBC Zürich')
    })
    ...
    it('maps awayTeam to encounter teamAway name', () => {
      ...
      expect(result.refereeGame?.game?.encounter?.teamAway?.name).toBe('Volley Luzern')
    })
  })
  ...
  describe('gender mapping', () => {
    ...
    it('maps mixed gender to undefined', () => {
      ...
      expect(result.refereeGame?.game?.group?.phase?.league?.gender).toBeUndefined()
    })
  ...
  describe('league category mapping', () => {
    it('maps leagueCategory to league leagueCategory name', () => {
      ...
      expect(result.refereeGame?.game?.group?.phase?.league?.leagueCategory?.name).toBe('NLA')
    })
    ...
    it('handles null leagueCategory', () => {
      ...
      expect(result.refereeGame?.game?.group?.phase?.league?.leagueCategory).toBeUndefined()
    })
  })
  ...
  describe('hall mapping', () => {
    ...
    it('maps address to hall postal address', () => {
      ...
      expect(result.refereeGame?.game?.hall?.primaryPostalAddress?.streetAndHouseNumber).toBe(
        'Hardaustrasse 10, 8005 Zürich'
      )
      expect(result.refereeGame?.game?.hall?.primaryPostalAddress?.city).toBe('Zürich')
    })
    ...
    it('maps coordinates to geographical location', () => {
      ...
      const geo = result.refereeGame?.game?.hall?.primaryPostalAddress?.geographicalLocation
  ...
  describe('referee convocation mapping', () => {
    it('maps referee1 to first head referee convocation', () => {
      ...
      expect(
        result.refereeGame?.activeRefereeConvocationFirstHeadReferee?.indoorAssociationReferee
    ...
    it('maps referee2 to second head referee convocation', () => {
      ...
      expect(
        result.refereeGame?.activeRefereeConvocationSecondHeadReferee?.indoorAssociationReferee
          ?.indoorReferee?.person?.displayName
      ).toBe('Anna Schmidt')
    })
    ...
    it('maps lineReferee1 to first linesman convocation', () => {
      ...
      expect(
        result.refereeGame?.activeRefereeConvocationFirstLinesman?.indoorAssociationReferee
          ?.indoorReferee?.person?.displayName
      ).toBe('Line Ref One')
    })
    ...
    it('maps lineReferee2 to second linesman convocation', () => {
      ...
      expect(
        result.refereeGame?.activeRefereeConvocationSecondLinesman?.indoorAssociationReferee
          ?.indoorReferee?.person?.displayName
      ).toBe('Line Ref Two')
    })
    ...
    it('handles empty referees object', () => {
      ...
      expect(result.refereeGame?.activeRefereeConvocationFirstHeadReferee).toBeUndefined()
      expect(result.refereeGame?.activeRefereeConvocationSecondHeadReferee).toBeUndefined()
    })
  })
...
describe('extractCityFromAddress', () => {
  it('extracts city from Swiss address with postal code and city', () => {
    expect(extractCityFromAddress('Bergstrasse 2, 8800 Thalwil')).toBe('Thalwil')
  })

  it('extracts city from address with street, postal code and city', () => {
    expect(extractCityFromAddress('Sternenfeldstrasse 50, 4127 Birsfelden')).toBe('Birsfelden')
  })

  it('extracts city from address with multi-word city name', () => {
    expect(extractCityFromAddress('Route de Valavran 10, 1293 Bellevue')).toBe('Bellevue')
  })
  ...
  it('returns last part when no postal code pattern found', () => {
    expect(extractCityFromAddress('Some Hall, Some Address, SomeCity')).toBe('SomeCity')
  })
  ...
  it('handles address with multiple commas', () => {
    expect(extractCityFromAddress('Sporthalle, Hauptstrasse 1, 3000 Bern')).toBe('Bern')
  })
})
