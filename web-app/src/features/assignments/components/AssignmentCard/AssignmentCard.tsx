...
import { detectSingleBallHall, getSingleBallHallsPdfPath } from '@/shared/utils/single-ball-halls'
...
function AssignmentCardComponent({
  ...
  const position = getPositionLabel(assignment.refereePosition, t, t('occupations.referee'))
  const gender = game?.group?.phase?.league?.gender

  // Referee names from the refereeGame
  const refereeGame = assignment.refereeGame
  const headReferee1 = getRefereeDisplayName(refereeGame?.activeRefereeConvocationFirstHeadReferee)
  const headReferee2 = getRefereeDisplayName(refereeGame?.activeRefereeConvocationSecondHeadReferee)
  const linesman1 = getRefereeDisplayName(refereeGame?.activeRefereeConvocationFirstLinesman)
  const linesman2 = getRefereeDisplayName(refereeGame?.activeRefereeConvocationSecondLinesman)
  const linesman3 = getRefereeDisplayName(refereeGame?.activeRefereeConvocationThirdLinesman)
  const linesman4 = getRefereeDisplayName(refereeGame?.activeRefereeConvocationFourthLinesman)
  ...
  return (
    <ExpandableCard
      ...
      renderCompact={(_, { expandArrow }) => (
        <AssignmentCardContext.Provider value={{ ...contextValue, expandArrow }}>
          <Header />
        </AssignmentCardContext.Provider>
      )}
      renderDetails={() => (
        <AssignmentCardContext.Provider value={{ ...contextValue, expandArrow: null }}>
          <Details />
        </AssignmentCardContext.Provider>
      )}
    />
