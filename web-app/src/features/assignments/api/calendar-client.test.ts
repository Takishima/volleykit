...
describe('calendar-client', () => {
  ...
  describe('calendarAssignmentToAssignment', () => {
    ...
    it('converts calendar assignment to API assignment format', async () => {
      ...
      vi.mocked(calendarApiModule.fetchCalendarAssignments).mockResolvedValue([calendarAssignment])
      ...
      expect(assignment.refereeGame?.game?.startingDateTime).toBe('2025-01-15T19:30:00+01:00')
      expect(assignment.refereeGame?.game?.encounter?.teamHome?.name).toBe('VBC Zürich')
      expect(assignment.refereeGame?.game?.encounter?.teamAway?.name).toBe('Volley Luzern')
      ...
      expect(assignment.refereeGame?.game?.hall?.primaryPostalAddress?.combinedAddress).toBe(
        'Hardaustrasse 10, 8005 Zürich'
      )
      expect(
        assignment.refereeGame?.game?.hall?.primaryPostalAddress?.geographicalLocation?.latitude
      ).toBe(47.3769)
    })
    ...
    it('maps men gender to m', async () => {
      ...
      const result = await calendarApi.searchAssignments()
      expect(result.items[0]?.refereeGame?.game?.encounter?.teamHome?.gender).toBe('m')
    })
    ...
    it('maps women gender to f', async () => {
      ...
      const result = await calendarApi.searchAssignments()
      expect(result.items[0]?.refereeGame?.game?.encounter?.teamHome?.gender).toBe('f')
    })
    ...
    it('maps mixed gender to undefined', async () => {
      ...
      const result = await calendarApi.searchAssignments()
      expect(result.items[0]?.refereeGame?.game?.encounter?.teamHome?.gender).toBeUndefined()
    })
    ...
    it('handles hall without coordinates', async () => {
      ...
      expect(
        result.items[0]?.refereeGame?.game?.hall?.primaryPostalAddress?.geographicalLocation
      ).toBeUndefined()
    })
  })
  ...
  describe('searchAssignments', () => {
    ...
    it('fetches and converts calendar assignments', async () => {
      ...
      expect(calendarApiModule.fetchCalendarAssignments).toHaveBeenCalledWith('ABC123')
    })
  })
  ...
  describe('getAssignmentDetails', () => {
    it('throws error when no calendar code', async () => {
      ...
      await expect(calendarApi.getAssignmentDetails('calendar-123', [])).rejects.toThrow(
        'Calendar code not available'
      )
    })
    ...
    it('throws error when assignment not found', async () => {
      ...
      await expect(calendarApi.getAssignmentDetails('calendar-99999', [])).rejects.toThrow(
        'Assignment not found: calendar-99999'
      )
    })
  })
  ...
  describe('getAssociationSettings', () => {
    it('returns default settings with referee edit hours', async () => {
      ...
      expect(settings.hoursAfterGameStartForRefereeToEditGameList).toBe(DEFAULT_REFEREE_EDIT_HOURS)
    })
  })
  ...
  describe('unsupported operations', () => {
    it('searchCompensations throws CalendarModeNotSupportedError', async () => {
      await expect(calendarApi.searchCompensations()).rejects.toThrow(CalendarModeNotSupportedError)
    })
    ...
    it('updateCompensation throws CalendarModeNotSupportedError', async () => {
      await expect(calendarApi.updateCompensation()).rejects.toThrow(CalendarModeNotSupportedError)
    })

    it('searchExchanges throws CalendarModeNotSupportedError', async () => {
      await expect(calendarApi.searchExchanges()).rejects.toThrow(CalendarModeNotSupportedError)
    })
    ...
    it('searchPersons throws CalendarModeNotSupportedError', async () => {
      await expect(calendarApi.searchPersons()).rejects.toThrow(CalendarModeNotSupportedError)
    })
    ...
    it('finalizeScoresheet throws CalendarModeNotSupportedError', async () => {
      await expect(calendarApi.finalizeScoresheet()).rejects.toThrow(CalendarModeNotSupportedError)
    })

    it('uploadResource throws CalendarModeNotSupportedError', async () => {
      await expect(calendarApi.uploadResource()).rejects.toThrow(CalendarModeNotSupportedError)
    })
