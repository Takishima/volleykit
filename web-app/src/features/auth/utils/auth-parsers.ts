...
export async function submitLoginCredentials(
  ...
  const isRedirectToDashboard =
    isRedirectResponse && locationHeader !== null && locationHeader.includes(DASHBOARD_URL_PATTERN)
  ...
  if (isRedirectToDashboard) {
    logger.info(
      'iOS PWA mode: Login successful (detected from 303 redirect), fetching dashboard...'
    )
