...
- The app's exact search request (above) has not been verified against the live API yet -
  the browser's own request body was never captured, only the response. A smoke test
  after deployment should confirm the server accepts a `searchConfiguration` without
  `propertyFilters` and still returns `_permissions` per item.
