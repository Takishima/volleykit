...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    build: {
      rollupOptions: {
        output: {
          ...
          //   - Total JS Bundle:                  ~679 kB, limit 700 kB (+21 kB headroom for CI, measured CI peak: 690.7 kB)
          manualChunks(id) {
    ...
    test: {
      ...
      // Derived from asyncUtilTimeout so the two cannot drift into equality.
      // See src/test/timeouts.ts.
