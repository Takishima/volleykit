...
describe('parseErrorResponse', () => {
  ...
  describe('JSON error responses', () => {
    ...
    it('falls back to status when JSON has no recognized fields', async () => {
      const response = new Response(JSON.stringify({ unknownField: 'some value' }), {
        status: 500,
    ...
    it('handles application/json with charset', async () => {
      const response = new Response(JSON.stringify({ message: 'Error with charset' }), {
        status: 400,
        statusText: 'Bad Request',
  ...
  describe('text error responses', () => {
    ...
    it('strips nested HTML tags to prevent bypass', async () => {
      const response = new Response("<<script>script>alert('xss')<</script>/script>", {
        status: 500,
        statusText: 'Internal Server Error',
        headers: { 'Content-Type': 'text/html' },
      })
  ...
  describe('edge cases', () => {
    it('handles very long error messages in JSON', async () => {
      ...
      const response = new Response(JSON.stringify({ message: longMessage }), {
        status: 400,
        statusText: 'Bad Request',
        headers: { 'Content-Type': 'application/json' },
      })
