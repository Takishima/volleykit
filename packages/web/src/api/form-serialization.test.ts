...
import { buildFormData, setCsrfToken, getCsrfToken, clearCsrfToken } from './form-serialization'
...
describe('form-serialization', () => {
  ...
  describe('circular reference detection', () => {
    it('throws error for direct circular reference', () => {
      ...
      expect(() => buildFormData(obj)).toThrow('Circular reference detected in form data')
    })
    ...
    it('throws error for indirect circular reference', () => {
      ...
      expect(() => buildFormData(objA)).toThrow('Circular reference detected in form data')
    })
    ...
    it('throws error for deeply nested circular reference', () => {
      ...
      expect(() => buildFormData(root)).toThrow('Circular reference detected in form data')
    })
  ...
  describe('MAX_DEPTH boundary testing', () => {
    ...
    it('throws error when exceeding MAX_DEPTH (11 levels)', () => {
      ...
      expect(() => buildFormData(data)).toThrow('Form data exceeds maximum nesting depth of 10')
    })
  ...
  describe('mixed-type array serialization', () => {
    ...
    it('serializes arrays with mixed objects and primitives', () => {
      const result = buildFormData({
        items: [{ type: 'object', id: 1 }, 'string-item', 42, { nested: { value: 'deep' } }],
      })
