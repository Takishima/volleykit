...
interface UseSwipeGestureOptions {
  ...
  onDragEnd?: (translateX: number, containerWidth: number) => boolean | Promise<boolean>
...
export function useSwipeGesture(options: UseSwipeGestureOptions = {}): UseSwipeGestureReturn {
  ...
  const threshold =
    thresholdOption ?? (containerWidth ? containerWidth * DEFAULT_SWIPE_THRESHOLD_RATIO : 100)
  ...
  const handleDragMove = useCallback(
    (clientX: number, clientY: number, preventDefault: () => void) => {
      ...
      if (directionRef.current === null) {
        if (Math.abs(diffX) > DIRECTION_THRESHOLD_PX || Math.abs(diffY) > DIRECTION_THRESHOLD_PX) {
      ...
      if (directionRef.current === 'horizontal') {
        ...
        const clampedTranslate = Math.max(-maxSwipe, Math.min(maxSwipe, newTranslate))
