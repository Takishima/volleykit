  ...
  ScorerSearchPanel: ({
    ...
    return (
      <div data-testid="scorer-search-panel">
        ...
        <div data-testid="selected-scorer">{selectedScorer?.displayName || 'none'}</div>
        {readOnlyScorerName && <div data-testid="read-only-scorer-name">{readOnlyScorerName}</div>}
        <button
        ...
        <button data-testid="clear-scorer-btn" onClick={() => onScorerSelect(null)}>
          Clear Scorer
        </button>
      </div>
    )
...
describe('ScorerPanel', () => {
  ...
  it('passes readOnlyScorerName to ScorerSearchPanel', () => {
    render(<ScorerPanel readOnly={true} readOnlyScorerName="Existing Scorer" />)

    expect(screen.getByTestId('read-only-scorer-name')).toHaveTextContent('Existing Scorer')
  })
  ...
  it('initializes with provided initialScorer', () => {
    ...
    expect(screen.getByTestId('selected-scorer')).toHaveTextContent('Max Mustermann')
  })

  it('calls onScorerChange with initialScorer on mount', async () => {
    const onScorerChange = vi.fn()

    render(<ScorerPanel initialScorer={mockScorer} onScorerChange={onScorerChange} />)
  ...
  it('maintains internal state when no onScorerChange is provided', async () => {
    ...
    expect(screen.getByTestId('selected-scorer')).toHaveTextContent('New Scorer')
  })
})
