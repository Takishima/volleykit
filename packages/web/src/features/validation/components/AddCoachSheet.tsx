...
function getRoleLabel(role: CoachRole, t: ReturnType<typeof useTranslation>['t']): string {
  switch (role) {
    case 'head':
...
export function AddCoachSheet({ isOpen, onClose, role, onSelectCoach }: AddCoachSheetProps) {
    ...
    <ResponsiveSheet isOpen={isOpen} onClose={handleClose} titleId="add-coach-title">
      ...
      <div className="flex-shrink-0 flex items-center justify-between p-4 border-b border-border-default dark:border-border-default-dark">
        <div>
          ...
          <p className="text-sm text-text-muted dark:text-text-muted-dark">{roleLabel}</p>
        </div>
        <button
          onClick={handleClose}
                  ...
                  >
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-3 text-sm text-text-muted dark:text-text-muted-dark">
                        {coach.associationId && <span>ID: {coach.associationId}</span>}
                        {coach.birthday && (
                          <span>
