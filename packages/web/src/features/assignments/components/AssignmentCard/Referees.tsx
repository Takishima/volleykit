...
export function Referees() {
  ...
  return (
    <div className="text-xs text-text-subtle dark:text-text-subtle-dark pt-1 space-y-0.5">
      {headReferee1 && (
        <div>
          <span className="font-medium">{t('positions.head-one')}:</span> {headReferee1}
        </div>
      )}
      {headReferee2 && (
        <div>
          <span className="font-medium">{t('positions.head-two')}:</span> {headReferee2}
        </div>
      )}
      {linesmen.length > 0 && (
        <div>
          <span className="font-medium">{t('occupations.linesmen')}:</span> {linesmen.join(', ')}
        </div>
      )}
    </div>
  )
