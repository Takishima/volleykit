...
export interface CalendarMappingsStore {
  ...
  getMapping(storage: StorageAdapter, assignmentId: string): Promise<CalendarEventMapping | null>
  /** Add or update a mapping */
  setMapping(storage: StorageAdapter, mapping: CalendarEventMapping): Promise<void>
 ...
 */
async function loadMappings(storage: StorageAdapter): Promise<CalendarEventMapping[]> {
  try {
...
export const calendarMappingsStore: CalendarMappingsStore = {
  ...
  async setMapping(storage, mapping) {
    const mappings = await loadMappings(storage)
    const index = mappings.findIndex((m) => m.assignmentId === mapping.assignmentId)
