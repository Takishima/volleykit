 ...
 * persisted before the property was requested may carry no value for it, so a
 * warm cache would keep rendering the pre-change result.
 *
 ...
 * 2: exchanges started requesting `_permissions`, which decides whether an entry
 *    is offered at all. Whether earlier builds already received it unasked is
 *    unconfirmed; a one-time cache discard is the cheaper side of that doubt.
 */
