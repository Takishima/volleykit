...
describe('Integration: Proxy Response Headers', () => {
  ...
  it('applies no-cache headers to JSON API responses', async () => {
    ...
    const request = new Request('https://proxy.example.com/indoorvolleyball.refadmin/api/test', {
      headers: {
        Origin: 'https://example.com',
      },
    })
  ...
  it('adds session warning header for 401 responses', async () => {
    ...
    expect(response.headers.get('X-Proxy-Session-Warning')).toBe('potential-session-issue')
  ...
  it('preserves iCal cache control (not dynamic content)', async () => {
    ...
    const mockICalResponse = new Response('BEGIN:VCALENDAR\nVERSION:2.0\nEND:VCALENDAR', {
      status: 200,
      headers: {
    ...
    const request = new Request('https://proxy.example.com/iCal/referee/ABC123', {
      headers: {
        Origin: 'https://example.com',
      },
    })
...
describe('Integration: Auth Lockout in Worker', () => {
  ...
  it('clears lockout on successful login', async () => {
    ...
    const mockSuccessResponse = new Response(null, {
      ...
      headers: {
        Location:
          'https://volleymanager.volleyball.ch/sportmanager.volleyball/main/dashboard?__csrftoken=abc',
      },
    })
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue(mockSuccessResponse))
...
describe('Session Token Relay', () => {
  describe('extractSessionCookies (simulated)', () => {
    ...
    it('handles multiple cookies', () => {
      const cookies = ['session=abc123; Path=/; Secure', 'csrf_token=xyz789; Path=/']
      const result = extractSessionCookies(cookies)
      expect(result).toBeDefined()
...
describe('iOS Safari Authentication Workaround', () => {
  ...
  it('rewrites POST /login with credentials to auth endpoint', async () => {
    ...
    expect(capturedUrl).toContain('/sportmanager.security/authentication/authenticate')
    expect(capturedBody).toContain('__authentication%5BNeos%5D%5BFlow%5D%5BSecurity%5D')
...
describe('Cookie Rewriting for Cross-Origin', () => {
  ...
  it('removes Domain attribute from cookies', async () => {
    ...
    const mockResponse = new Response(JSON.stringify({ data: 'test' }), {
      ...
      headers: {
        'Content-Type': 'application/json',
        'Set-Cookie': 'session=abc123; Domain=.volleyball.ch; Path=/; HttpOnly',
      },
    })
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue(mockResponse))
...
describe('Redirect Handling', () => {
  ...
  it('rewrites internal redirects to go through proxy', async () => {
    ...
    const request = new Request('https://proxy.example.com/sportmanager.volleyball/main/home', {
      headers: {
        Origin: 'https://example.com',
      },
    })
