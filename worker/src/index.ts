...
async function processWithTextract(
  ...
  const lines =
    result.Blocks?.filter((block) => block.BlockType === "LINE").map(
      (block) => block.Text || "",
    ) || [];
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    // Provider selection via OCR_PROVIDER env var (default: "mistral")
    if (url.pathname === "/ocr") {
      const origin = request.headers.get("Origin");
      let allowedOrigins: string[];
      try {
      ...
      if (provider === "mistral" && !env.MISTRAL_API_KEY) {
        return new Response(
          JSON.stringify({ error: "Mistral OCR service not configured" }),
          {
            status: 503,
            headers: {
              "Content-Type": "application/json",
      ...
      if (
        ...
        return new Response(
          JSON.stringify({ error: "AWS Textract service not configured" }),
          {
            status: 503,
            headers: {
              "Content-Type": "application/json",
