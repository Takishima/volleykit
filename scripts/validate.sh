...
#
# `ocr-poc` is in pnpm-workspace.yaml but deliberately absent here: it is a
# throwaway proof of concept with no lint/test/build scripts worth gating.
...
# a prettier config change has nothing to do with whether the mobile tests
# still pass, and treating it as repo-wide re-runs the entire monorepo for it.
#
...
SHELL_INPUTS=".claude/hooks .claude/settings.json scripts/validate.sh scripts/validation-lib.sh scripts/validate.test.sh scripts/shellcheck.sh .github/workflows/ci-shell.yml"

# The design-token check compares the two generated files. Trigger and inputs
# are the same constant so editing the generator both invalidates the cache and
# registers the check.
...
# them before the gate reopens. The trigger fires on SHELL_INPUTS or on any
# changed `*.sh` file wherever it lives; the changed files enter the cache key
# too, so a stored PASS cannot be reported as a hit for a check that was only
# just triggered.
...
if [ -n "$CHANGED_SHELL" ] || matches "$(paths_to_regex "$SHELL_INPUTS")"; then
  ...
  # (.github/workflows/ci-shell.yml). scripts/shellcheck.sh is the single
...
# Run one check. CHECK_CMD is space-separated argv; two composite commands
