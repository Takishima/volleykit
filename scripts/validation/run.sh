...
# stated here rather than inherited from the caller's source order. The load
# is top level in a top-level-sourced file, and re-sourcing is harmless — the
# lib defines functions and re-assigns its constants.
# shellcheck source=./lib.sh
source "$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/lib.sh" || return 1

# Run one check. CHECK_CMD is either space-separated argv (exec'd without a
# shell) or a `__sentinel__` resolving to a composite_* function.
run_check() {
  local name=$1
  ...
  case "${CHECK_CMD[$name]}" in
    __*)
      # Registration has already verified the function exists.
      local fn="composite_${CHECK_CMD[$name]//__/}"
      "$fn" "$name" >"$out" 2>&1 && ok=0
      ;;
    *)
      # `read -ra` splits on IFS and does no glob expansion; the registration
      # guard has already rejected metacharacters and globs.
      local -a cmd=()
