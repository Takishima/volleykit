...
# fingerprinting, path-set matching (paths_to_regex), the check cache and the
# exec-bit invariant. Project policy (which packages exist, which checks they
# run) lives in scripts/validation/policy.sh; selection and the gate in
# scripts/validate.sh.
#
# This file is meant to be sourced. It does not set shell options, does not
# change the caller's working directory, does not read policy constants and
# does not exit the caller — it only defines functions and exports ROOT_DIR,
# and reports failure through return codes. Source it from a directory inside
# the repository; every function below assumes the caller has cd'd to ROOT_DIR.
#
...
# Print the non-empty lines of a newline-separated path list — the one idiom
# for iterating CHECK_PATHS / CORE_ROOT_NL entries without IFS-splitting
...
paths_to_regex() {
  local out="" p
  # shellcheck disable=SC2086  # our own constant: split on purpose
  for p in $1; do
    p=${p//\\/\\\\}
    p=${p//./\\.}
    p=${p//+/\\+}
...
# Every tracked file under the given pathspecs must carry mode 100755.
#
# For files invoked by path (the Claude Code hooks, scripts/validate.sh in the
# permission allowlist) a dropped exec bit means the file never runs and never
# emits a decision — the one failure a fail-closed hook cannot report. A mode
# is not content, so no fingerprint can carry it: the caller must run this
# uncached, on every invocation. Pure git, so it works wherever the repository
# does. Untracked files have no tracked mode and are out of scope by
