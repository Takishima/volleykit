...
6. **Given** multiple assignments on the same day at different venues (>500m apart), **When** departure checks run, **Then** each assignment gets its own notification with venue-specific transit details
7. **Given** multiple assignments on the same day at nearby venues (≤500m apart), **When** departure checks run, **Then** a single grouped notification is sent for the cluster
...
- **FR-026b**: System MUST handle multiple assignments per day with individual notifications for distant venues (>500m apart) and grouped notifications for nearby venues (≤500m apart)
...
- Q: How are multiple assignments on the same day handled? → A: Individual notification per assignment, unless venues are within 500m of each other (then grouped into single notification). Notifications suppressed if user already within 500m of venue.
