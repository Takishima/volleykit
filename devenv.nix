...
}:
{
  ...
  packages = with pkgs; [
    ...
    curl
    jq
    # scripts/validate.sh registers a shell-lint check when this is present.
    shellcheck
  ];
  ...
  git-hooks.hooks = {
    ...
    # commit. Human developers rely on CI; the Claude Code commit gate lives
    # in .claude/hooks/pre-git-commit.sh.
    #
